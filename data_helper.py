...
def loadDataset_predict(filename):
    '''
    读取样本数据
    :param filename: 文件路径，是一个字典，包含word2id、id2word分别是单词与索引对应的字典和反序字典，
                    trainingSamples样本数据，每一条都是QA对
    ...
    with open(dataset_path, 'rb') as handle:
        ...
        word2id = data['word2id']
        id2word = data['id2word']
        predictSamples = data['predictSamples']
    return word2id, id2word, predictSamples
...
def load_dict_from_file(filepath):
    ...
    return _dict, sentence_list, id_list

def build_txtsamples(filename):
    sen2id, _, _ = load_dict_from_file(filename)
    sentence_list = []
...
def build_p_input(filename):
    sen2id, sentence_list, id_list = load_dict_from_file(filename)
    # sentence_list = []
    # id_list = []
    # for sen in sen2id.keys():
    #     sentence_list.append(sen)
...
def build_flags(data):
    ...
    for sample in data:
        ...
        q = sample[1]
        for i in p:
            p_flags.append(1 if i in q else 0)
        for j in q:
            q_flags.append(1 if j in p else 0)
...
def sample_pad(samples, labels):
    ...
    for label in labels:
        ...
        if label == '0':
            ...
            targets.append([0, 1])
    return p_inputs, q_inputs, targets, p_lengths, q_lengths
...
def loadDataset_pad(filename):
    ...
    print('Loading dataset from {}'.format(dataset_path))
    with open(dataset_path, 'rb') as handle:
        data = pickle.load(handle)  # Warnibuild_txtsamplesng: If adding something here, also modifying saveDataset
        word2id = data['word2id']
        id2word = data['id2word']
...
if __name__ == '__main__':
    ...
    # flags = loadDataset_flags('data/train_data_flags.pkl')
    p_inputs, q_inputs, targets, p_len, q_len = sample_pad(trainingSamples, trainingLabels)
    # p_inputs_char, q_inputs_char = sample_char_pad(trainingSamples_char)
    # p_flags, q_flags = sample_flags_pad(flags)
    all_data = {'p_inputs':np.array(p_inputs), 'q_inputs':np.array(q_inputs), 'targets':np.array(targets), 'p_len':np.array(p_len),
    ...
    output_file = open('data/train_data_pad_len.pkl', 'wb')
    pickle.dump(all_data, output_file)
    # output_file_char = open('data/train_data_char_pad.pkl', 'wb')
    # pickle.dump(all_data_char, output_file_char)
    # output_file_flags = open('data/train_data_flags_pad.pkl', 'wb')
    ...
    # word2id, id2word, trainingSamples, trainingLabels = loadDataset_test('data/dev_data_n.pkl')
    # samples_flags = build_flags(trainingSamples)
    # print(samples_flags)
    # all_data = {'testingSamples_flags': samples_flags}
    # output_file = open('data/dev_data_flags.pkl', 'wb')
    # pickle.dump(all_data, output_file)
    ...
    # output_file = open('data/train_data_flags.pkl', 'wb')
    # pickle.dump(all_data, output_file)
    ...
    # trainingSamples = build_char_data(char2id, 'data/dev.tsv')
    # print(trainingSamples)
    # all_data = {'char2id': char2id, 'id2char': id2char,
    #            'testingSamples_char': trainingSamples}
    # output_file = open('data/dev_data_char.pkl', 'wb')
    # pickle.dump(all_data, output_file)
    ...
    # word2id, id2word = load_dictionary()
    # print(word2id)
    # trainingSamples, trainingLabels = process_data(word2id, 'data/train.tsv')
    # print(len(trainingSamples))
    # all_data = {'word2id': word2id, 'id2word': id2word,
    #            'trainingSamples': trainingSamples,
    #            'trainingLabels': trainingLabels}
    # output_file = open('data/train_data_n.pkl', 'wb')
    # pickle.dump(all_data, output_file)

    # testingSamples, testingLabels = process_data(word2id, 'data/test.tsv')
    # all_data = {'word2id': word2id, 'id2word': id2word,
    #             'testingSamples': testingSamples,
    #             'testingLabels': testingLabels}
    # output_file = open('data/test_data_n.pkl', 'wb')
    # pickle.dump(all_data, output_file)
