...
#             # print(old_name)
#             os.system('ffmpeg -i '+s_path_+old_name+' -map 0:5 '+srt_parent_+'S'+str(i+1)+'-'+str(p+1)+'.srt')
#             p += 1
...
####### 删除mkv文件
for i in range(10):
    p2_path = 'Friends/S'+str(i+1)
    file_list = os.listdir(p2_path)
    for file_name in file_list:
        reg = re.compile(r'mkv$')
        if reg.search(file_name):
            os.remove(p2_path+'/'+file_name)
